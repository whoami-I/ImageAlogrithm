...
import kotlinx.android.synthetic.main.activity_adjust_gamma.*
...
class GammaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        seekbar_s.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                tv_s.text = (seekbar_s.progress / 30.0f).toString()
            }
            ...
            override fun onStopTrackingTouch(seekBar: SeekBar?) {
                val v = seekBar!!.progress / 30.0f
                val progressDialog = ProgressDialog(this@GammaActivity)
                progressDialog.show()
                progressDialog.setCancelable(false)
                lifecycleScope.launch(Dispatchers.IO) {
                    val canvas = Canvas(tmpBitmap!!)
                    canvas.drawBitmap(srcBitmap!!, 0.0f, 0.0f, null)
                    AlgManager.gamma(v, tmpBitmap!!)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        uri?.also {
            lifecycleScope.launch(Dispatchers.IO) {
                val futureBitmap = Glide.with(this@GammaActivity)
                    .asBitmap()
                    .load(it)
                    .submit()
